...
export class AccountTransactionsHeader extends React.PureComponent<Props> {
  render () {
    ...
    const totalBalance = convertNQTStringToNumber(account.lastBalance);
    ...
    return (
        <View style={styles.view}>
          <Text textAlign={TextAlign.CENTER} color={Colors.WHITE} size={FontSizes.LARGE} bold>
            {i18n.t(core.currency.BURST.value, { value: amountToString(totalBalance) })}
          </Text>
          {priceInBTC ? (
              <Text textAlign={TextAlign.CENTER} color={Colors.WHITE}>
