...
export class TransactionListItem extends React.PureComponent<Props> {
  ...
  isOwnAccount (address: string | undefined): boolean {
    return address && address === this.props.address ? true : false;
  }
  ...
  getAmount = (transaction: Transaction): number => {
    ...
    return this.isMultiOutPayment(transaction)
      ? getRecipientsAmount(convertAddressToNumericId(this.props.address), transaction)
  ...
  render () {
    ...
    const amount = this.getAmount(this.props.transaction).toString();
    let address = isNegative ? recipientRS : senderRS;

    if (this.isMultiOutPayment(this.props.transaction)) {
      address = 'Multi-out Payment';
    }
        ...
        <View style={styles.mainView}>
          ...
          <View style={styles.dataView}>
            {isNegative ? (
                <View style={styles.outcomingAmount}>
                  <View style={styles.flex}>
                    <Text color={Colors.RED}>{amount}</Text>
                  </View>
                  <View style={styles.flex}>
            ...
            <View style={styles.account}>
              <Text color={Colors.WHITE}>{address}</Text>
            </View>
            {!isNegative ? (
                <View style={styles.incomingAmount}>
                  ...
                  <View style={styles.flex}>
                    <Text color={Colors.GREEN}>{amount}</Text>
                  </View>
                </View>
            ) : null}
          </View>
